...
Fetches index 15-min candles, detects descending channel patterns, backtests, shows chart + results.
"""
...
def render_channel_down_tab(container, index_name="NIFTY"):
    """Build the Channel Down historical backtester tab for given index."""
    with container:
        ui.label(f"Channel Down Scanner — {index_name} 15-min").classes(
        ...
        with content_container:
            ...
            ui.label(f"Loading {index_name} Channel Down data...").classes(
    ...
    async def refresh():
        try:
            ...
            if content_container.client._deleted:
                return
            _build_channel_down_content(content_container, index_name, candles)
            ...
            print(f"  [channel_down:{index_name}] error:\n{traceback.format_exc()}")

    return refresh


def _build_channel_down_content(container, index_name, candles):
