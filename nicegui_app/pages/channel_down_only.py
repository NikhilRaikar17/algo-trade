"""
Channel Down historical backtest tab page.
Fetches 15-min candles for any NSE index or stock, detects descending channel patterns, backtests.
"""
...
from algo_strategies import detect_channel_down_signals, backtest_channel_down
from tv_charts import render_tv_channel_down_chart, flush_pending_js
...
def render_channel_down_tab(container):
    """Build the Channel Down historical backtester tab. Returns an async refresh() closure."""

    selected = {"security_id": _DEFAULT_SEC_ID, "label": _DEFAULT_LABEL}
    live_options = dict(_ALL_OPTIONS)

    with container:
        ui.label("Channel Down Scanner").classes("text-xl font-bold mb-2")
        with ui.element("div").classes(
            "bg-red-50 border border-red-200 rounded-lg px-4 py-2 mb-3"
        ):
            ui.label(
                "Strategy: Bearish descending channel — two lower highs form upper trendline | "
                "Entry: Trendline rejection close | Target: Lower trendline | SL: Upper trendline + 0.5% | "
                "15-min candles | 5 days"
        ...
        with content_container:
            ...
            ui.label(f"Loading {_DEFAULT_LABEL} Channel Down data...").classes(
    ...
    async def _load(security_id: str, label: str):
        ...
        with content_container:
            ui.spinner("dots", size="lg").classes("mx-auto my-8")
            ui.label(f"Loading {label} Channel Down data...").classes(
                "text-gray-500 text-center w-full"
            )
        ...
        try:
            ...
            if content_container.client._deleted:
                return
            try:
                _build_channel_down_content(content_container, label, candles)
            ...
            try:
                ...
                return
            print(f"  [channel_down:{label}] error:\n{traceback.format_exc()}")
...
def _build_channel_down_content(container, label, candles):
    """Detect channel down patterns, backtest, render charts and tables."""
    container.clear()
    with container:
        if candles.empty:
        ...
        trades = backtest_channel_down(signals, candles)

        # --- Chart ---
        ui.label(
            f"{label} — Last: {candles['close'].iloc[-1]:,.2f} | "
            f"{len(candles)} candles (15-min, 5 days) | "
            f"{len(signals)} channel down signal{'s' if len(signals) != 1 else ''}"
        ).classes("text-md font-semibold mb-2")
        render_tv_channel_down_chart(candles, signals)

        # --- Summary ---
        if not trades:
            ui.label("No channel down patterns detected in this period.").classes(
            ...
            rows.append(
                {
                    ...
                    "H1":             t["H1"],
                    "H2":             t["H2"],
                    "Upper @ Entry":  t["upper_at_signal"],
                    "Lower @ Entry":  t["lower_at_signal"],
                    "Width":          t["channel_width"],
                    "Entry":          t["entry"],
                    "Target":         t["target"],
                    "SL":             t["stop_loss"],
        ...
        columns = [
            ...
            {"name": "h1",           "label": "H1",            "field": "H1",            "sortable": True, "align": "left"},
            {"name": "h2",           "label": "H2",            "field": "H2",            "sortable": True, "align": "left"},
            {"name": "upper_entry",  "label": "Upper @ Entry", "field": "Upper @ Entry", "sortable": True, "align": "left"},
            {"name": "lower_entry",  "label": "Lower @ Entry", "field": "Lower @ Entry", "sortable": True, "align": "left"},
            {"name": "width",        "label": "Width",         "field": "Width",         "sortable": True, "align": "left"},
