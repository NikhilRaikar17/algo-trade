...
Fetches index 15-min candles, detects double top patterns, backtests, shows chart + results.
"""
...
def render_double_top_tab(container, index_name="NIFTY"):
    """Build the Double Top historical backtester tab for given index."""
    with container:
        ui.label(f"Double Top Scanner — {index_name} 15-min").classes(
        ...
        with content_container:
            ...
            ui.label(f"Loading {index_name} Double Top data...").classes(
    ...
    async def refresh():
        try:
            ...
            if content_container.client._deleted:
                return
            _build_double_top_content(content_container, index_name, candles)
            ...
            print(f"  [double_top:{index_name}] error:\n{traceback.format_exc()}")

    return refresh


def _build_double_top_content(container, index_name, candles):
